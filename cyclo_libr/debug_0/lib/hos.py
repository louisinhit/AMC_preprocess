...
class HOC_cyclic:
    def __init__(self, window_size, step, res=1, fs_fc=False):
    ...
    def cmf(self, s, p, q):
        ...
        suanzi = cp.outer(cp.asarray(self.al), cp.asarray(self.wt))
        suanzi = cp.exp(-2j*cp.pi*suanzi)  # size alpha * window
        cmf = cp.einsum('nki,ji->nkj', cp.asarray(s), suanzi) / cp.asarray(self.window_size)
