...
hoc = hos.HOC_cyclic(window_size=512, step=128)

fi = open("pics/logbook_rdctcf.txt", "w")
fi = open("pics/logbook_rdctcf.txt", "a")
fi.write("Start \n")
...
def image(label, cm):
    fig = plt.figure()
    name = "pics/predict_rdctcf_SNR_{}".format(label)
...
def tocsv(label, cm):
    df = pd.DataFrame(cm)
    df.to_csv("pics/predict_rdctcf_{}.csv".format(label))
...
fi.writelines('test rdctcf, high SNR. \n')
print ('start test')
...
fi.writelines('test rdctcf, middle SNR. \n')
print ('start test')
...
fi.writelines('test rdctcf, low SNR. \n')
print ('start test')
