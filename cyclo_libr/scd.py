...
fi = open("pics/logbook_scd.txt", "w")
fi = open("pics/logbook_scd.txt", "a")
fi.write("Start \n")
...
def image(label, cm):
    fig = plt.figure()
    name = "pics/predict_scd_SNR_{}".format(label)
...
def tocsv(label, cm):
    df = pd.DataFrame(cm)
    df.to_csv("pics/predict_scd_{}.csv".format(label))
...
def run(snr):
    ...
    for i in mod:
        for j in snr:
            ...
            s = cyc.scf_fam(s)
            ccsd0.append(block_reduce(s[:tr, :, :], block_size=(1, 1, 8), func=np.max).reshape((tr, -1)))
            ccsd1.append(block_reduce(s[tr::, :, :], block_size=(1, 1, 8), func=np.max).reshape((te, -1)))
...
fi.writelines('test scd, high SNR. \n')
print ('start test')
...
fi.writelines('test scd, middle SNR. \n')
print ('start test')
...
fi.writelines('test scd, low SNR. \n')
print ('start test')
