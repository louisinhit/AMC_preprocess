...
def block_samples(s, window_size, step):
    ...
    return torch.as_strided(s, (s.shape[0], no, window_size), (L, step, 1))import math, torch
import sys, time
from torch.fft import fft, fftshift
import numpy as np
...
def CHTC(s, ws, step, d):
    ...
    #u, _, __ = torch.linalg.svd(o.transpose(-2, -1), full_matrices=True)
    #o = torch.einsum('mjk,mkl->mjl', o, u[:, :, :d])
    # normalize
    mi = torch.amin(o, dim=(1, 2), keepdims=True)
    o = (o - mi) / (torch.amax(o, dim=(1, 2), keepdims=True) - mi)
